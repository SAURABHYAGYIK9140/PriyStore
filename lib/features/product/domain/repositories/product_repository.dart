...
class ProductRepository implements ProductRepositoryInterface {
  ...
  @override
  // Future<ApiResponse> getBrandOrCategoryProductList(bool frombanner,bool isBrand, String id) async {
  //   try {
  //     String uri;
  //     if(isBrand){
  ...
  Future<ApiResponse> getBrandOrCategoryProductList(
      String catd, String frombanner, bool isBrand, String id) async {
    try {
      String uri;

      // if (frombanner == true) {
      //   uri = '${AppConstants.categoriesUri}$id?guest_id=1''&is_dhakad=1&parent_id=' + catd;
      // }
      if (isBrand) {

        if (frombanner == "dhakad") {
          // uri = '${AppConstants.brandProductUri}$id?guest_id=1';
          uri = '${AppConstants.brandProductUri}$id?guest_id=1''&is_bundle=1&parent_id=' + catd;

        }else if (frombanner == "false") {
          uri = '${AppConstants.brandProductUri}$id?guest_id=1''&is_dhakad=1&parent_id=' + catd;
        }
        else{
          uri = '${AppConstants.brandProductUri}$id?guest_id=1';
        }
        ...
        if (frombanner == "dhakad") {
          uri = '${AppConstants.categoriesUri}$id?''&is_bundle=1&parent_id=' + catd;

        }else if (frombanner == "false") {
          uri = '${AppConstants.categoriesUri}$id?''&is_dhakad=1&parent_id=' + catd;
        }
        else{
          uri = '${AppConstants.categoryProductUri}$id?guest_id=1';
      ...
      print(uri);
      final response = await dioClient!.get(uri);
      print('brandProductUri' + response.toString());
      print('brandProductUri' + response.requestOptions.uri.toString());
  ...
  @override
  Future<ApiResponse> getDeal2prolist(String offset) async {
    try {
      final response = await dioClient!.get(
        AppConstants.get_deal_of_the_day_product + offset,
      );
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
  ...
  @override
  Future<ApiResponse> getRecentProductList(String offset) async {
    try {
      final response = await dioClient!.get(
        AppConstants.recentProductUri + offset,
      );
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
  ...
  @override
  Future<ApiResponse> getR2mendedProductList(String offset) async {
    try {
      final response = await dioClient!.get(
        AppConstants.recomendedProductUri + offset,
      );
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
