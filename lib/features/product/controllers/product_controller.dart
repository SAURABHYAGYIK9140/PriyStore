...
class ProductController extends ChangeNotifier {
  ...
  List<StreepModel>? _streepprodlist = [];
  List<Product>? _latestProductList = [];
  List<Product>? _Deal2ProductList = [];
  List<Product>? _recentproduct = [];
  List<Product>? _recommendedproduct = [];
  List<Product>? _dealofday = [];
  ...
  List<String> _Deal2OffsetList = [];
  List<String> _rOffsetList = [];
  List<String> _rm2OffsetList = [];
  List<String> _dealoffeset = [];
  List<String> get lOffsetList=>_lOffsetList;
  List<String> get Deal2OffsetList=>_Deal2OffsetList;
  List<String> get rOffsetList=>_rOffsetList;
  List<String> get rm2OffsetList=>_rm2OffsetList;
  List<String> get dealoffeset=>_dealoffeset;
  List<String> _featuredOffsetList = [];

  List<Product>? get latestProductList => _latestProductList;
  List<Product>? get Deal2ProductList => _Deal2ProductList;
  List<StreepModel>? get streepprodlist => _streepprodlist;
  List<Product>? get recentproduct => _recentproduct;
  List<Product>? get recommendedproduct => _recommendedproduct;
  List<Product>? get dealofday => _dealofday;
  ...
  //Deal2 product
  Future<void> getDeal2prolist(String offset, {bool reload = false}) async {
    print("getDeal2prolist");

    if(reload) {
      _Deal2OffsetList = [];
      _Deal2ProductList = [];
    }
    if(!_Deal2OffsetList.contains(offset)) {
      _Deal2OffsetList.add(offset);
      ApiResponse apiResponse = await productServiceInterface!.getDeal2prolist(offset);
      if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
        _Deal2ProductList = [];
        _Deal2ProductList?.addAll(ProductModel.fromJson(apiResponse.response!.data).products??[]);
        _lPageSize = ProductModel.fromJson(apiResponse.response!.data).totalSize;
        _firstLoading = false;
        _isLoading = false;
        print("_Deal2ProductList"+_Deal2ProductList!.length.toString());
      } else {
        ApiChecker.checkApi( apiResponse);
      }
      notifyListeners();
  ...
  Future<void> fetchProducts() async {
    {
      print("streplist");
      streepprodlist?.clear();
      notifyListeners();
      final String apiUrl = AppConstants.baseUrl+"/api/v1/products/hometopstrip?guest_id=1&limit=10&offset=";

      final response = await http.get(Uri.parse(apiUrl));

      if (response.statusCode == 200) {
        List<dynamic> body = jsonDecode(response.body);
        List<StreepModel> streplist = body.map((dynamic item) => StreepModel.fromMap(item)).toList();
        print("streplist"+streplist[0].products.length.toString());
        streepprodlist?.addAll(streplist);
        print("streplist"+streplist.length.toString());
        notifyListeners();
      } else {
  ...
  Future<void> getRecent(String offset, {bool reload = false}) async {
    print("getRecentgetRecentgetRecentgetRecent");

    if(reload) {
      _rOffsetList = [];
      _recentproduct = [];
    }
    if(!_rOffsetList.contains(offset)) {
      
      _rOffsetList.add(offset);
      ApiResponse apiResponse = await productServiceInterface!.getRecentProductList(offset);
      if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
        _recentproduct = [];
        _recentproduct?.addAll(ProductModel.fromJson(apiResponse.response!.data).products??[]);
        _lPageSize = ProductModel.fromJson(apiResponse.response!.data).totalSize;
        _firstLoading = false;
        _isLoading = false;
      } else {
  ...
  Future<void> getDealoftheday(String offset, {bool reload = false}) async {
    print("getDealoftheday");

    if(reload) {
      _dealoffeset = [];
      _dealofday = [];
    }
    if(!_dealoffeset.contains(offset)) {
      _dealoffeset.add(offset);
      ApiResponse apiResponse = await productServiceInterface!.getRecommendedProductList(offset);
      if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
        _dealofday = [];
        _dealofday?.addAll(ProductModel.fromJson(apiResponse.response!.data).products??[]);
        _lPageSize = ProductModel.fromJson(apiResponse.response!.data).totalSize;
        _firstLoading = false;
        _isLoading = false;
      } else {
  ...
  Future<void> getRecomended(String offset, {bool reload = false}) async {
    print("getRecomended");
    ...
    if(!_rm2OffsetList.contains(offset)) {
      _rm2OffsetList.add(offset);
      ApiResponse apiResponse = await productServiceInterface!.getRecommendedProductList(offset);
      if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
        _recommendedproduct = [];
        _recommendedproduct?.addAll(ProductModel.fromJson(apiResponse.response!.data).products??[]);
        _lPageSize = ProductModel.fromJson(apiResponse.response!.data).totalSize;
        _firstLoading = false;
        _isLoading = false;
      } else {
...
int selectedProductTypeIndex = 0;
 void changeTypeOfProduct(ProductType type, String? title, {int index = 0}){
  ...
  void initBrandOrCategoryProductList(String catid,String frombanner,bool isBrand, String id, BuildContext context) async {
    _brandOrCategoryProductList.clear();
    print("initBrandOrCategoryProductList");

    _hasData = true;
    ApiResponse apiResponse = await productServiceInterface!.getBrandOrCategoryProductList(catid,frombanner
        ,isBrand, id);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
  ...
  final List<CategoryModel> _homeCategoryProductList = [];
  List<CategoryModel> get homeCategoryProductList => _homeCategoryProductList;
      ...
      if (apiResponse.response != null  && apiResponse.response!.statusCode == 200) {
        if(apiResponse.response!.data.toString() != '{}'){
          ...
          apiResponse.response!.data.forEach((homeCategoryProduct) =>
              _homeCategoryProductList.add(CategoryModel.fromJson(homeCategoryProduct)));
        }
      } else {
        ApiChecker.checkApi( apiResponse);
      }
