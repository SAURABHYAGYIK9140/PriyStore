...
class BrandController extends ChangeNotifier {
  ...
  List<BrandModel> _brandListmain = [];
  List<BrandModel> _brandList = [];
  List<BrandModel> get brandListmain => _brandListmain;
  List<BrandModel> get brandList => _brandList;
  final List<BrandModel> _originalBrandList = [];

  Future<void> getBrandListmain(bool reload) async {
    if (_brandListmain.isEmpty || reload) {
      ...
      _brandListmain.clear();
        apiResponse.response!.data.forEach((brand) => _brandListmain.add(BrandModel.fromJson(brand)));
